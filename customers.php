...
$db = getDbInstance();
$select = array('id', 'company', 'emp', 'phone', 'created_at', 'updated_at', 'email');

if ($search_string) {
    $db->where('company', '%' . $search_string . '%', 'like');
    $db->orwhere('emp', '%' . $search_string . '%', 'like');
}
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="page-action-links text-right">
                <a href="add_customer.php?operation=create" class="btn btn-success">Adicionar novo cliente <i class="glyphicon glyphicon-plus"></i></a>
            </div>
        </div>
    </div>
  ...
  ");
    ...
    <script>
        ...
        const data = {
            labels: labels,
            datasets: [{
                label: 'Número de clientes registados por mês',
    ...
    <table class="table table-striped table-bordered table-condensed">
        <thead>
            <tr>
                ...
                <th width="25%">Nome do responsável</th>
                <th width="20%">Telemóvel</th>
                <th width="17%">Ação</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($rows as $row) : ?>
                <tr>
                    ...
                    <td><?php echo xss_clean($row['emp']); ?></td>
                    <td><?php echo xss_clean($row['phone']); ?></td>
                    <td>
                        <a href="mailto:<?php echo xss_clean($row['email']) ?>" class="btn btn-success"><i class="fa fa-envelope"></i></a>

                        <a href="view_customer.php?customer_id=<?php echo $row['id']; ?>" class="btn btn-warning"><i class="glyphicon glyphicon-user"></i></a>
                        ...
                        <form action="delete_customer.php" method="POST">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <p>Tem a certeza que quer apagar este cliente?</p>
                                </div>
